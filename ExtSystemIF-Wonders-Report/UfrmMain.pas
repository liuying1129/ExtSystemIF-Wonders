...
begin
  ...
  ADOTemp22.Close;
  ADOTemp22.SQL.Clear;
  //万达HIS扫码的检验单特征:chk_con.His_MzOrZy的值不为空(等于0或1)                                                                                                                                         
  ADOTemp22.SQL.Text:='select * from view_Chk_Con_All cc where isnull(cc.Weight,'''')='''' and isnull(cc.report_doctor,'''')<>'''' and isnull(cc.His_MzOrZy,'''') in (''0'',''1'',''3'') and isnull(cc.His_Unid,'''')<>'''' and Audit_Date>GETDATE()-7 and isnull(TjJianYan,'''')<>'''' ';
  ADOTemp22.Open;
